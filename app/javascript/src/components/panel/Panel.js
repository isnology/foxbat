      ...
      <div className="configurator">
        <div className="panel-main">
          ...
          <div className="running-cost">
            { !!store.panelName ? <p>Panel: <i>{ store.panelName }</i></p> :
              <i style={ { color: '#bdbdbd' } }>Save to name your panel</i> }
            <p>Current cost (USD): ${ numeral(totalCost()).format('0,0.00') }</p>
          </div>
          
          <div className={`panel ${store.template.substring(0,3)} ${store.template.length > 3 ? "digital" : ""}`}>
            { store.template === 'a22' || store.template === 'a22Digital' ? <A22outline/> : <A32outline/> }
            { slotLayout[store.template].map((slotArray, index) => (
              <div key={ index } className={ slotArray[0].substring(0, 1).toLowerCase() + "-container" }>
                { slotArray.map((slot, index2) => (
                  <Slot
                    ...
                    onSelectSlot={ store.onSelectSlot }
                    pxWidth={ store.pxWidth }
                  />
                ))
                }
              </div>
          ...
          <div className="panel-button-group">
            { store.signedIn &&
              <Button onClick={ store.onSave }
                text="Save"
                style={ saveButtonStyle }
              />
            }
            { store.signedIn &&
              <SubmitButton onClick={ store.submitPanel }
                className="panel-button-group"
                email={ store.email }
                slots={ store.slots }
                template={ store.template }
                templateSlots={ store.templateSlots }
              />
            }
            <div className="panel-button-low-group">
              { !store.signedIn &&
                <Button onClick={ () => store.onModalWindow("signIn") }
                  text="Sign In"
                />
              }
              { store.signedIn &&
              ...
              <Button onClick={ store.onClearCurrentPanel }
                text={ "Clear panel" }
              />
              { store.signedIn && !!store.panelId &&
                <Button onClick={ store.onDeletePanel }
                  text="Delete panel"
                />
              }
              <Button onClick={ () => store.onRefreshApp(true) }
                text="Back to start"
              />
            </div>
          </div>
        ...
        <Sidebar
          ...
          template={ store.template }
          state={ this.state }
          onSelectInstrument={ store.onSelectInstrument }
          onUpdateSlots={ store.onUpdateSlots }
          onSelectInstrumentClass={ store.onSelectInstrumentClass }
          onSelectInstrumentBrand={ store.onSelectInstrumentBrand }
          onSidebarClose={ store.onSidebarClose }
        />
        
        { store.modalWindow === "save" &&
          <Save
            onExit={ store.onExit }
            onSubmit={ store.onSave }
